# -*- coding: utf-8 -*-
"""
Полный сравнительный эксперимент: 7 методов — COVID-19 Radiography Database.
Задача: COVID-19 (редкий класс) vs Normal (мажоритарный класс).
Условия: при отсутствии многометочных данных вектор условий обнулён,
это делает DDPM безусловным, сохраняя архитектуру.
Методы:
1. Baseline (SimpleCNN)
2. Cost-Sensitive (Focal Loss + SimpleCNN)
3. SMOTE + MLP
4. WGAN-GP + SimpleCNN
5. WGAN-GP + ResNet-18
6. DDPM (безусловный) + SimpleCNN
7. DDPM (безусловный) + ResNet-18 (улучшенный)

Дополнительно сохраняются: примеры двух классов, сравнение реальных и сгенерированных изображений.
Вся печать и подписи на русском языке.
"""
...
DATA_BASE = '/kaggle/input/datasets/tawsifurrahman/covid19-radiography-database/COVID-19_Radiography_Dataset'
MAJOR_CLASS = 'Normal'
MINOR_CLASS = 'COVID'
SELECTED_DISEASES = ['Lung_Opacity']  # размерность условия 1, все значения 0
...
NUM_SYNTHETIC = 200
MAX_MAJOR_SAMPLES = 2800        # число примеров класса Normal
MAX_MINOR_SAMPLES = 400         # число примеров класса COVID-19
DIFFUSION_TIMESTEPS = 200

# Загрузка данных
def load_covid_data(base_dir, major_class='Normal', minor_class='COVID',
                    max_major=2800, max_minor=400, selected_diseases=['Lung_Opacity']):
    data = []
    normal_folder = os.path.join(base_dir, major_class, 'images')
    if not os.path.exists(normal_folder):
        normal_folder = os.path.join(base_dir, major_class)
    for img_file in os.listdir(normal_folder):
        if img_file.lower().endswith(('.png', '.jpg', '.jpeg')):
            data.append({'image_path': os.path.join(normal_folder, img_file), 'label': 0})
    covid_folder = os.path.join(base_dir, minor_class, 'images')
    if not os.path.exists(covid_folder):
        covid_folder = os.path.join(base_dir, minor_class)
    for img_file in os.listdir(covid_folder):
        if img_file.lower().endswith(('.png', '.jpg', '.jpeg')):
            data.append({'image_path': os.path.join(covid_folder, img_file), 'label': 1})
    df = pd.DataFrame(data)
    df_major = df[df['label'] == 0]
    if len(df_major) > max_major:
        df_major = df_major.sample(n=max_major, random_state=seed)
    df_minor = df[df['label'] == 1]
    if len(df_minor) > max_minor:
        df_minor = df_minor.sample(n=max_minor, random_state=seed)
    df = pd.concat([df_major, df_minor], ignore_index=True)
    for disease in selected_diseases:
        df[disease] = 0
    return df

print("Загрузка данных COVID-19 Radiography Database...")
df = load_covid_data(DATA_BASE, MAJOR_CLASS, MINOR_CLASS,
                     max_major=MAX_MAJOR_SAMPLES, max_minor=MAX_MINOR_SAMPLES,
...
# Визуализация примеров двух классов
def show_class_examples(dataframe, class_labels, save_path, n=3):
    fig, axes = plt.subplots(1, len(class_labels), figsize=(4*len(class_labels), 4))
    for idx, (label, name) in enumerate(class_labels):
        samples = dataframe[dataframe['label'] == label].sample(n, random_state=seed)
        sample_img = Image.open(samples.iloc[idx]['image_path']).convert('RGB')
        axes[idx].imshow(sample_img.resize((256,256)))
        axes[idx].set_title(name, fontsize=14)
        axes[idx].axis('off')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Примеры классов сохранены в {save_path}")

class_labels = [(0, 'Норма'), (1, 'COVID-19')]
show_class_examples(df, class_labels, '/kaggle/working/class_examples_covid.png')
...
def save_comparison_image(real_loader, fake_images_tensor, save_path, label_name):
...
# Главный эксперимент
print("\n" + "="*80)
print("Начало эксперимента: COVID-19 Radiography Database (условия обнулены)")
print("="*80)
...
class_weights = 1.0 / class_counts
class_weights = class_weights / class_weights.sum() * 2
print(f"Веса классов: Норма={class_weights[0]:.2f}, COVID-19={class_weights[1]:.2f}")
...
y_prob = mlp.predict_proba(X_test_feat)[:,1]
acc = accuracy_score(y_test_feat, y_pred)
rec = recall_score(y_test_feat, y_pred, average='binary')
prec = precision_score(y_test_feat, y_pred, average='binary')
f1 = f1_score(y_test_feat, y_pred, average='binary')
auc = roc_auc_score(y_test_feat, y_prob)
results["SMOTE + MLP"] = (acc, rec, prec, f1, auc)
print(f"SMOTE результаты: Acc={acc*100:.2f}%, Rec={rec*100:.2f}%, Prec={prec*100:.2f}%, F1={f1*100:.2f}%, AUC={auc*100:.2f}%")

# 4. WGAN-GP (безусловный) обучение на редком классе
print("\n--- 4. Обучение WGAN-GP ---")
...
syn_imgs_uncond = []
with torch.no_grad():
    for _ in range(NUM_SYNTHETIC // BATCH_SIZE + 2):
...
print(f"Сгенерировано {len(syn_imgs_cond)} изображений DDPM")

save_comparison_image(minor_train_loader, syn_imgs_cond,
                      '/kaggle/working/comparison_covid_ddpm.png', 'COVID-19')

# Фильтрация синтетических изображений
print("\nФильтрация синтетических изображений...")
filter_model = PretrainedCNN(num_classes=2).to(device)
filter_model = train_classifier(filter_model, train_loader_raw, val_loader_tensor,
                                epochs=10, model_name="Filter", lr=0.001, use_focal=False, patience=3)
filter_model.eval()

def filter_images(imgs, model, threshold=0.8):
    model.eval()
    kept = []
    with torch.no_grad():
        for i in range(0, len(imgs), 64):
            ...
            kept.append(imgs[i:i+64][mask])
    if kept:
        return torch.cat(kept, dim=0)
    else:
        return torch.empty(0,3,IMG_SIZE,IMG_SIZE)

syn_imgs_uncond = filter_images(syn_imgs_uncond, filter_model, threshold=0.8)
syn_imgs_cond = filter_images(syn_imgs_cond, filter_model, threshold=0.8)
print(f"После фильтрации: безусловных {len(syn_imgs_uncond)}, DDPM {len(syn_imgs_cond)}")

if len(syn_imgs_uncond) == 0:
    print("Предупреждение: не осталось безусловных изображений, используются оригинальные.")
if len(syn_imgs_cond) == 0:
    print("Предупреждение: не осталось DDPM изображений, используются оригинальные.")

# Аугментированные данные
if len(syn_imgs_uncond) > 0:
    X_aug_uncond = torch.cat([X_train, syn_imgs_uncond], dim=0)
    y_aug_uncond = torch.cat([y_train, torch.ones(len(syn_imgs_uncond)).long()], dim=0)
else:
    X_aug_uncond, y_aug_uncond = X_train, y_train
...
if len(syn_imgs_cond) > 0:
    ...
    y_aug_cond = torch.cat([y_train, torch.ones(len(syn_imgs_cond)).long()], dim=0)
else:
    X_aug_cond, y_aug_cond = X_train, y_train
...
acc, rec, prec, f1, auc, _, _ = evaluate_model(modelA, test_loader_tensor)
results["WGAN-GP + SimpleCNN"] = (acc, rec, prec, f1, auc)
...
acc, rec, prec, f1, auc, _, _ = evaluate_model(modelB, test_loader_tensor)
results["WGAN-GP + ResNet-18"] = (acc, rec, prec, f1, auc)
...
acc, rec, prec, f1, auc, _, _ = evaluate_model(modelE, test_loader_tensor)
results["DDPM (безусловный) + SimpleCNN"] = (acc, rec, prec, f1, auc)

# 9. DDPM + ResNet-18 (улучшенный)
print("\n--- 9. DDPM + ResNet-18 (улучшенный) ---")
...
acc, rec, prec, f1, auc, _, _ = evaluate_model(modelF, test_loader_tensor)
results["DDPM (безусловный) + ResNet-18 (улучш.)"] = (acc, rec, prec, f1, auc)
...
axes = axes.flatten()
model_info = [
    ("Baseline (SimpleCNN)", evaluate_model(model_baseline, test_loader_tensor)),
    ("Cost-Sensitive (Focal Loss + SimpleCNN)", evaluate_model(model_cost, test_loader_tensor)),
    ("SMOTE + MLP", (acc, rec, prec, f1, auc, y_test_feat, y_pred)),
    ("WGAN-GP + SimpleCNN", evaluate_model(modelA, test_loader_tensor)),
    ("WGAN-GP + ResNet-18", evaluate_model(modelB, test_loader_tensor)),
    ("DDPM (безусл.) + SimpleCNN", evaluate_model(modelE, test_loader_tensor)),
    ("DDPM (безусл.) + ResNet-18 (улучш.)", evaluate_model(modelF, test_loader_tensor))
]
...
plt.suptitle("Сравнение методов: COVID-19 Radiography Database", fontsize=14)
plt.tight_layout()
plt.savefig('/kaggle/working/confusion_matrices_covid.png', dpi=300, bbox_inches='tight')
plt.show()
